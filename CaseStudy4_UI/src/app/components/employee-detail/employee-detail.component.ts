...
import { SalarySlip } from '../models/salary-slip';
...
export class EmployeeDetailComponent implements OnInit {
  ngOnInit(): void {
    ...
    this.activatedRoute.paramMap.subscribe((params) => {
      if (params.get('id')){
        ...
        this.getSalarySlip(this.routeId);
      }
    })
  }
  ...
  getSalarySlip(id: number){
    this.http.get<SalarySlip>(`http://localhost:5003/api/stake-holder-employees/employees/${id}/salary-slip`)
    .subscribe({
      next: (res) => {
        this.salarySlip = res
      },
